...
function filterCourses() {
    ...
    rows.each(function () {
        const collegeId = $(this).find('td').eq(0); // Fix index to match
													// "Course ID"
        const collegeName = $(this).find('td').eq(1); // Add matching for
														// "Course Name"

        if (collegeId.length && collegeName.length) {
            // Check if the Course ID or Course Name matches the search input
            if (
            		collegeId.text().toLowerCase().includes(searchInput) ||
            		collegeName.text().toLowerCase().includes(searchInput)
            ) {
